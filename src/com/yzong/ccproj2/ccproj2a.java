...
public class ccproj2a {
	...
	public static void main(String[] args) throws IOException,
		...
		System.out.println("Time elapsed: "
				+ (endMain.getTime() - startMain.getTime()) + " ms.");
		return;
	}
	 ...
	 * @throws InterruptedException
	 */
	public static void runBenchmark(String instanceType) throws IOException,
			InterruptedException {
		...
		for (int i = 0; i < totalRounds; i++) {
			System.out.println("Benchmarking Round " + (i + 1) + " out of "
					+ totalRounds + ".");
			Process childProcess = null;
			ProcessBuilder pb = new ProcessBuilder(scriptName, imgName,
					totalReqCount, parallelCount, instanceDNSName, logPrefix
							+ i);
		...
		getMetricStatisticsRequest
				.withStartTime(childStart)
				.withEndTime(childEnd)
				.withMetricName("CPUUtilization")
				.withPeriod(60)
				.withNamespace("AWS/EC2")
				.withUnit("Percent")
				.withStatistics("Average", "Maximum", "Minimum", "SampleCount",
						"Sum");
		GetMetricStatisticsResult getMetricStatResponse = cloudWatchClient
		...
		Collections.sort(dataPoints, new Comparator<Datapoint>() {
			public int compare(Datapoint dp1, Datapoint dp2) {
				return (int) (dp1.getTimestamp().getTime() - dp2.getTimestamp()
						.getTime());
			}
		});
		...
		System.out.print("Benchmark complete for " + instanceType
